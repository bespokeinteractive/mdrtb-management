<% 
	ui.decorateWith("appui", "standardEmrPage", [title: "Edit Budget"])
	ui.includeJavascript("mdrtbdashboard", "moment.js")
%>

<script>
	jq(function () {
		jq('.textable input').change(function() {
			var val = jq(this).val();
			var sum = 0;
			var tot = 0;
			var parent = "";
		...
		var confirmDialog = emr.setupConfirmationDialog({
            ...
            actions: {
                confirm: function () {
                    ...
                    jq.ajax({
                        type: "POST",
                        url: '${ui.actionLink("mdrtbmanagement", "financebudget", "updateBudget")}',
                        data: dataString,
                        dataType: "json",
		...
		jq('#addBudget').click(function(){
			...
			jq('.dialog-content .confirmation').html("Confirm editing the Budget for <b>" + jq('#facility option:selected').text() + " Facility</b> worth <b>&dollar;" + jq('#totals').val()+"</b> ?");
    ...
    <form class="patient-header new-patient-header">
        <div class="demographics">
            <h1 class="name" style="border-bottom: 1px solid #ddd;">
                <span><i class="icon-paste small"></i>EDIT BUDGET &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</span>
            </h1>
        </div>
		...
		<div class="budget-box">
			<div>
				${ui.includeFragment("uicommons", "field/datetimepicker", [formFieldName: 'budget.date', id: 'date-created', label: 'Date:', useTime: false, defaultDate: budget.dated])}
				<input type="hidden" name="budget.id" value="${budget.id}"/>
			</div>
			
			<div style="width: 63%; float: right">
				<label style="display: inline-block">Notes</label>
				<textarea name="budget.description" style="min-width: 0;display: inline-block;margin-top: 5px;">${budget.description}</textarea>
			</div>
			...
			<div>
				...
				<select id="facility" name="budget.facility">
					<% locations.eachWithIndex { loc, index -> %>
						<option value="${loc.id}" ${loc==budget.location?'selected':''} '>${loc.name}</option>
					<% } %>
				</select>
			</div>
				...
				<% charts.eachWithIndex { chart, index -> %>
					<% if (chart.hasChildren) { %>
						...
						<% chart.children.eachWithIndex { child, idx -> %>
							<tr>
								...
								<td>0.00</td>
								<td class="textable"><input type="text" class="child" data-uuid="${chart.id}" name="item.${child.id}" value="${child.value>0?child.value+'0':''}"/></td>
								<td>0.00</td>
							</tr>						
						<% } %>
					...
					<% } else { %>
						<tr>
							...
							<td>${chart.code}</td>
							<td>0.00</td>
							<td class="textable"><input type="text" name="item.${chart.id}" value="${chart.value>0?chart.value+'0':''}"/></td>
							<td>0.00</td>
						</tr>
					<% } %>
				<% } %>
